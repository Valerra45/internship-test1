namespace Task1
{
    ...
    public class Game
    {
        ...
        {
            var maxOffet = this.gameStamps.Max(x => x.offset);

            if (maxOffet < offset)
            {
                return this.gameStamps[^1].score;
            }

            int resultIdx = 0;

            for (var i = 0; i < this.gameStamps.Length; i++)
            {
                if (offset - gameStamps![i].offset == 0)
                {
                    resultIdx = i;

                    break;
                }
                else if (offset - gameStamps![i].offset < 0)
                {
                    resultIdx = i - 1;
            ...
            return this.gameStamps![resultIdx].score;
        }
    }
}